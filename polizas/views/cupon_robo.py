...
# 🚀 IMPORTAMOS EL NUEVO MODELO DE COMISION
from polizas.models import Poliza, CuponRobo, Comision
...
class CuponRoboViewSet(viewsets.ModelViewSet):
    ...
    def _registrar_egreso_y_comision(self, cupon: CuponRobo, request) -> None:
        """
        🚀 NUEVO MOTOR FINANCIERO:
        1. Registra el egreso de la caja hacia la compañía de seguros.
        2. Calcula y crea la comisión para el vendedor externo si existe.
        """
        monto_pagado_cliente = getattr(cupon, "monto", None)
        if not monto_pagado_cliente:
            return

        try:
            if float(monto_pagado_cliente) <= 0:
                return
        except Exception:
            return
            
        # 🚀 1. EXTRAEMOS EL COSTO DE LA COMPAÑÍA DESDE EL FRONTEND
        costo_compania_raw = request.data.get("costo_compania", 0)
        try:
            costo_compania = float(costo_compania_raw)
        except (ValueError, TypeError):
            costo_compania = 0.0

        fecha_egreso = cupon.fecha_pago.date() if getattr(cupon, "fecha_pago", None) else timezone.localdate()

        poliza = getattr(cupon, "poliza", None)
        poliza_num = getattr(poliza, "numero_poliza", "") if poliza else "s/n"
        patente = getattr(poliza, "patente", "") if poliza else "Sin Patente"
        
        # OBTENEMOS EL USUARIO Y LA OFICINA
        oficina_obj = getattr(poliza, "oficina", None)
        usuario_obj = getattr(self.request, "user", None) if self.request else None

        categoria = "Egreso a Compañía"
        descripcion_txt = f"Pago cupón de robo - Póliza {poliza_num} ({patente})"
        ...
        try:
            ...
            with transaction.atomic():
                ...
                # 🚀 2. CREAMOS EL EGRESO POR LO QUE LE PAGAMOS A LA COMPAÑÍA
                egreso = Egreso.objects.create(
                    descripcion=descripcion_txt,
                    monto=costo_compania if costo_compania > 0 else monto_pagado_cliente,
                    fecha=fecha_egreso,
                    categoria=categoria,
                    oficina=oficina_obj,      
                    usuario=usuario_obj,      
                    forma_pago="TRANSFERENCIA"   # 🚀 FIX: las cuponeras SIEMPRE se pagan por transferencia
                )
                
                # 🚀 3. MOTOR DE COMISIONES
                # Solo calculamos comisión si la póliza tiene un vendedor y hubo ganancia
                if poliza and poliza.vendedor and costo_compania > 0:
                    ganancia = float(monto_pagado_cliente) - costo_compania
                    
                    if ganancia > 0:
                        # Buscamos la cuota asociada a este cupón por fechas para enlazar la comisión
                        from pagos.models import Cuota
                        cuota_asociada = Cuota.objects.filter(
                            poliza=poliza, 
                            fecha_vencimiento__month=cupon.fecha_vencimiento.month,
                            fecha_vencimiento__year=cupon.fecha_vencimiento.year
                        ).first()
            ...
            try:
                _hist_log(
                    poliza=poliza,
                    tipo="CUPON_ROBO_EGRESO",
                    mensaje="Registrado egreso por pago de cupón de robo",
                    severidad="ACTION",
                    data={
                        "cupon_id": cupon.id,
                        "egreso_id": getattr(egreso, "id", None),
                        "monto_cliente": float(monto_pagado_cliente),
                        "costo_compania": costo_compania,
                        "oficina": oficina_obj.nombre if oficina_obj else "Sin oficina"
                    },
                    request=self.request,
                    subject=cupon,
                    categoria="EGRESO",
            ...
            print(f"ERROR AL CREAR EGRESO DE CUPÓN O COMISIÓN: {e}")
            try:
                _hist_log(
                    poliza=poliza,
                    tipo="CUPON_ROBO_EGRESO_ERROR",
                    mensaje="Error al crear egreso o comisión por cupón de robo",
                    severidad="ERROR",
                    data={"cupon_id": cupon.id, "error": str(e)},
                    request=self.request,
                    subject=cupon,
                    categoria="EGRESO",
                )
